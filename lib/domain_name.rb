...
class DomainName
  ...
  class << self
    ...
    def normalize(domain)
      sanitized_domain = domain.chomp(DOT)

      if sanitized_domain.respond_to?(:unicode_normalize)
        sanitized_domain = sanitized_domain.unicode_normalize(:nfc)
      end

      DomainName::Punycode.encode_hostname(sanitized_domain).downcase
    end
  end
end
